...
* Source Code Repository
  ...
  * Removes shared base image, opting for shared practices and patterns

* Image Builds
  * Can be built directly from a static `Containerfile` with no additional inputs or tooling
...
* Dependency versions
  * Commands generated in Jinja2 Macros
  * Dynamically defined
::::
...
* Each product has `Minimal` & `Standard` images ^[`workbench-session` & `connect-content` not yet available; see next slide]
  ...
  * `Standard`: "Traditional" image - `Minimal` plus optional dependencies such as additional system libraries, Python, R, Quarto, TinyTeX, etc.
