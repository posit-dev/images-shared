...
SOCI options can be set in the `options` array of an [Image](#image) or an [ImageVariant](#imagevariant). When both are present, the variant’s options are merged over the image’s options, with any field not explicitly set on the variant inheriting the image-level value. SOCI conversion runs as a post-build step via `bakery imagetools soci-convert` (standalone) or as part of `bakery ci publish` (config-driven), and only acts on targets where `enabled` is `true`.
