...
| `dependencyConstraints`<br/>*[DependencyConstraint](#dependencyConstraint) array*       | List of dependencies to install in the image. Versions are calculated from a [VersionConstraint](#versionconstraint)                       | `[]`                                      | <pre>- dependency: python<br/>  constraint:<br/>    latest: true<br/>    count:2</pre>                |
...
Pass `--recent N` to `bakery build` or `bakery ci matrix` to limit a
non-matrix image to its N highest-sorted release versions. `N` must be a
positive integer. Development versions are not counted against the limit, and
matrix images are not limited.

The shared GitHub Actions build workflows accept a `recent` input and pass it
to `bakery ci matrix` as `--recent N`.

`--recent` is applied before `bakery build --latest`. If the version marked
`latest: true` is outside the selected recent releases, no target is selected
...
| `tagAliases`<br/>*string array*      | Additional variant names to render wherever `{{ Variant }}` is used. Aliases retain the variant's primary filtering behavior.                                    | `[]`                                                                  | <pre>- default</pre>                                                                     |
...
| `extraRegistries`<br/>*[Registry](#registry) or [BaseRegistry](#baseregistry) array*    | Additional registries to push this image version to in addition to the global `registries` in [bakery.yaml](#bakery-configuration) and `extraRegistries` or `overrideRegistries` if set in the parent [Image](#image). Cannot be set with `overrideRegistries`.     | `[]`                                                                                                                            | <pre>- host: docker.io<br/>  namespace: posit</pre>                            |
...
By default, the following tag patterns will be used for an [Image](#image) if no `tagPatterns` are otherwise specified for the [Image](#image) or [ImageVariant](#imagevariant). These patterns mirror the behavior noted in the [Image Tags](index.qmd#image-tags) section of the README.
...
| Field                                | Description                                                                                                                                      | Default Value                | Example                  |
|--------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------|--------------------------|
| `tool`<br/>*"soci" literal string*   | *(Required)* The name of the tool.                                                                                                               |                              | `soci`                   |
| `enabled`<br/>*bool*                 | Enable SOCI conversion for this target. Targets without `enabled: true` are skipped during conversion.                                           | `false`                      | `true`                   |
| `span_size`<br/>*int*                | SOCI zTOC span size, in bytes. If unset, the SOCI default is used.                                                                               | SOCI default                 | `4194304`                |
| `min_layer_size`<br/>*int*           | Minimum layer size, in bytes, for a layer to be indexed. Smaller layers are skipped. If unset, the SOCI default is used.                         | SOCI default                 | `10485760`               |
| `prefetch_files`<br/>*string array*  | Files to mark for prefetch in the SOCI index.                                                                                                    | `[]`                         | <pre>- /opt/app/bin/server</pre> |
| `optimizations`<br/>*string array*   | Optional SOCI optimizations to apply (e.g. `xattr`).                                                                                             | `[]`                         | <pre>- xattr</pre>       |
| `platforms`<br/>*string array*       | Platforms to convert. If unset, all platforms in the source image are converted (`--all-platforms`).                                             | All platforms                | <pre>- linux/amd64<br/>- linux/arm64</pre> |
