...
def matrix(
    ...
    try:
        ...
        # Generate targets using the standard selection logic
        ...
        if selection is not None:
            ...
            if recent is not None:
                for img in c.model.images:
                    ...
                    included_versions = {t.image_version.name for t in targets if t.image_name == img.name}
                    for version in release_versions:
                        if version.name in cs.versions and version.name not in included_versions:
                            log.warning(
