...
from posit_bakery.targets.selection import exit_if_no_targets
...
class DGossPlugin(BakeryToolPlugin):
    ...
    def results(self, results: list[ToolCallResult]) -> None:
        ...
        if report_collection.test_failures:
            ...
            stderr_console.print("❌ dgoss test(s) failed", style="error")
        if has_errors:
            stderr_console.print("-" * 80)
            for err in errors:
                stderr_console.print(err, style="error")
            stderr_console.print("❌ dgoss command(s) failed to execute", style="error")
        if report_collection.test_failures or has_errors:
            raise typer.Exit(code=1)

        stderr_console.print("✅ Tests completed", style="success")
