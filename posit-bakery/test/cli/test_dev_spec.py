...
class TestRunDgossDevSpec:
    """Tests for --dev-spec / BAKERY_DEV_SPEC in the deprecated bakery run dgoss."""

    def _invoke(self, extra_args: list[str], env: dict | None = None):
        with (
            patch("posit_bakery.cli.run.BakeryConfig") as mock_config,
            patch("posit_bakery.cli.run.get_plugin") as mock_get_plugin,
        ):
            instance = MagicMock()
            instance.base_path = Path(BASIC_CONTEXT)
            instance.targets = []
            ...
            result = runner.invoke(
                app,
                ["run", "dgoss", "--context", BASIC_CONTEXT] + extra_args,
                env=env,
                catch_exceptions=False,
            )
            return result, mock_config
    ...
    def test_dev_spec_via_flag(self):
        """--dev-spec JSON is parsed and forwarded to BakerySettings in run dgoss."""
        result, mock = self._invoke(
            ["--dev-versions", "only", "--dev-spec", '{"version": "2026.05.0-dev+185-gSHA", "channel": "daily"}']
        )
        assert result.exit_code == 0, result.output
    ...
    def test_dev_spec_via_env_var(self):
        """BAKERY_DEV_SPEC env var works in run dgoss."""
        result, mock = self._invoke(
            ["--dev-versions", "only"],
            env={"BAKERY_DEV_SPEC": '{"version": "2026.05.0-dev+185-gSHA"}'},
        )
