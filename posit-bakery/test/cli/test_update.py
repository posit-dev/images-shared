...
@then(parsers.parse('the version "{version}" does not exist'))
@then(parsers.parse('the version "{version}" does not exist in the "{subpath}" subpath'))
def check_not_version(cli_test_tmpcontext, version, cli_test_image_name, subpath=None) -> str:
    config = BakeryConfig.from_context(cli_test_tmpcontext)
    image = config.model.get_image(cli_test_image_name)
...
def check_matrix_rendered_templates(
    ...
    version_dir = cli_test_tmpcontext / cli_test_image_name / DEFAULT_MATRIX_SUBPATH
