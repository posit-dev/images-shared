...
func (m *Manager) CopyProDriversOdbcInstIni() error {
	...
	if isFile {
		slog.Info("Backing up original odbcinst.ini to " + fmt.Sprintf("%s.bak", systemOdbcInstIniPath))
	...
	if err := file.Copy(positDriversOdbcInstIniPath, systemOdbcInstIniPath); err != nil {
		slog.Error(fmt.Sprintf("Failed to copy %s to %s. Original odbcinst.ini will be restored.", positDriversOdbcInstIniPath, systemOdbcInstIniPath))
		...
		return fmt.Errorf("unable to copy Pro Drivers example ini file from %s to %s: %w", positDriversOdbcInstIniPath, systemOdbcInstIniPath, err)
	}
